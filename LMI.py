...
from BayesianGMM import BayesianGaussianMixtureModel
...
class LMI(BaseIndex):
    ...
    """
    def __init__(self, PATH="./Mtree", labels=["L1", "L2"], knn_gts="knn_gt.json", desc_values = -1):
        super().__init__(PATH, labels, knn_gts, desc_values = desc_values)
        """
        Parameters
        ----------
        [TBD]
    ...
    def train(self, df, model_dict, pretrained_root=False, should_shuffle_dataset=True, should_erase=True, na_label=None):
        ...
        elif "BayesianGMM" in model_dict:
            self.classifier = BayesianGaussianMixtureModel()
            model, predictions, encoder = self.classifier.train(model_dict["BayesianGMM"][0], X, y, self.descriptor_values)
        elif "KMeans" in model_dict:
            self.classifier = KMeansModel()
            model, predictions, encoder = self.classifier.train(model_dict["KMeans"][0], X, y, self.descriptor_values)
        elif "Faiss" in model_dict:
            self.classifier = Faiss()
            model, predictions, encoder = self.classifier.train(model_dict["Faiss"][0], X, y, self.descriptor_values)
        else:
    ...
    def train_level(self, df, groupby_df, model_dict, level=1):
        ...
        for name, group in groupby_df:
            ...
            if "RF" in model_dict:
                ...
                model, predictions, encoder = self.classifier.train(model_dict["BayesianGMM"][level], X, y, self.descriptor_values)
            elif "KMeans" in model_dict:
                model, predictions, encoder = self.classifier.train(model_dict["KMeans"][level], X, y, self.descriptor_values)
            elif "Faiss" in model_dict:
