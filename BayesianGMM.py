...
class BayesianGaussianMixtureModel(BaseClassifier):
    ...
    def train(self, bayesian_gmm, X, y, descriptor_values):
        ...
        n_comp = bayesian_gmm["comp"]
        if X.shape[0] <= bayesian_gmm["comp"]:
            previous_bayesian_gmm_comp = bayesian_gmm["comp"]
            #bayesian_gmm["comp"] = X.shape[0]# // 2
            logging.warn(f"Reducing the number of components from {previous_bayesian_gmm_comp} to {X.shape[0]} since the number of training samples ({X.shape[0]}) is less than {previous_bayesian_gmm_comp}")
            n_comp = X.shape[0]
        root = BayesianGaussianMixture(n_components=n_comp, covariance_type=bayesian_gmm["cov_type"], max_iter=max_iter, init_params=init_params, weight_concentration_prior_type=weight_concentration_prior_type)
